...
class LMStudioManager(LLMWrapper):
    ...
    def send(self, user_message: str, system_prompt: int = 0) -> str:
        """
        ...
        Args:
            ...
            system_prompt (int, optional): The index or identifier for the system prompt to use. Defaults to 0.
        Returns:
        ...
        try:
            ...
            self._logger.info(f"Received reply from LM Studio: {reply}")
    ...
    def decide(self, question: str, response_format: dict, system_prompt: int = 0, retries: int = 3) -> str:
        """
        ...
        self._logger.debug(f"system_prompt: {system_prompt}")
        sys_p_text = ""
        if system_prompt is not None:
            sys_p_text = f'{self.get_system_prompt(system_prompt)}\n\n'
        ...
        for attempt in range(1, retries + 1):
            ...
            try:
                ...
                self._logger.debug(f"Raw response content: {response.choices[0].message.content}")
